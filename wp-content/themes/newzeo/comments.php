...
<?php /* Run some checks for bots and password protected posts */ ?>
    ...
    <div class="nopassword"><?php _e('This post is password protected. Enter the password to view any comments.', 'newzeo') ?></div>
</div><!-- .comments -->
...
<?php if ( have_comments() ) : ?>	<?php /* Count the number of comments and trackbacks (or pings) */	$ping_count = $comment_count = 0;		foreach ( $comments as $comment )			get_comment_type() == "comment" ? ++$comment_count : ++$ping_count; ?>
			...
			<div id="comments-list" class="comments">
				...
				<?php endif; ?>                  
				<?php /* An ordered list of our custom comments callback, newzeo_comments(), in functions.php   */ ?>
				<ol>					<?php wp_list_comments('type=comment&callback=newzeo_comments'); ?>
				</ol>
		...
		<?php if ( ! empty($comments_by_type['pings']) ) : ?>
				...
				<div id="trackbacks-list" class="comments">
					<h3><?php printf($ping_count > 1 ? __('<span>%d</span> Trackbacks', 'newzeo') : __('<span>One</span> Trackback', 'newzeo'), $ping_count) ?></h3>
